        ...
        <div className="w-full max-w-full min-w-0 overflow-hidden bg-white rounded-3xl p-4 sm:p-5 lg:p-6 border border-slate-200/80 shadow-sm mb-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-10 min-w-0">
            ...
            <div className="lg:col-span-5 min-w-0">
              <div className="aspect-square w-full max-w-full rounded-2xl bg-slate-100 overflow-hidden border border-slate-200 relative group">
                ...
                {product.image ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 bg-sky-50 px-4 text-center">
                    <Package className="w-12 h-12 sm:w-16 sm:h-16 mb-2 stroke-[1.5]" />
                    <span className="text-sm font-semibold">
                      Rural Handcrafted Item
                    </span>
                  </div>
                )}

                {product.category?.name && (
                  <span className="absolute top-3 left-3 sm:top-4 sm:left-4 max-w-[calc(100%-1.5rem)] truncate bg-slate-900/90 text-white text-[10px] sm:text-xs font-bold px-2.5 sm:px-3 py-1 rounded-full uppercase tracking-wider backdrop-blur-sm">
                    {product.category.name}
                  </span>
                )}
            ...
            {/* Right Column */}
            <div className="lg:col-span-7 min-w-0 flex flex-col justify-between gap-6">
              ...
              <div className="min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3 min-w-0">
                  ...
                  <div className="shrink-0 flex items-center gap-1 bg-amber-50 border border-amber-200 px-2.5 py-1 rounded-lg text-amber-700 font-bold text-xs">
                    <Star className="w-3.5 h-3.5 shrink-0 fill-amber-500 text-amber-500" />
                    <span>4.9 Rating</span>
                  </div>

                  <span className="max-w-full truncate text-xs font-semibold text-sky-600 bg-sky-50 px-2.5 py-1 rounded-lg border border-sky-100">
                    100% Handcrafted
                  </span>
                ...
                {/* Title */}
                <h1 className="min-w-0 max-w-full wrap-break-word text-2xl sm:text-3xl font-black text-slate-900 tracking-tight leading-snug">
                  {product.name}
                </h1>

                {/* Price */}
                <div className="mt-4 flex flex-wrap items-baseline gap-x-3 gap-y-1 min-w-0">

                  <span className="text-3xl sm:text-4xl font-extrabold text-slate-900 shrink-0">
                    ₹{Number(product.price).toLocaleString('en-IN')}
                  </span>

                  <span className="text-xs text-slate-500 wrap-break-word">
                    Taxes included • Free Rural Shipping
                  </span>

                </div>

                {/* Stock Status */}
                <div className="mt-4 p-3 rounded-xl bg-slate-50 border border-slate-200/80 min-w-0">
                  ...
                  {outOfStock ? (
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2 text-xs font-semibold text-slate-700 mb-1">
                      ...
                      <div className="w-full max-w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                        <div
                          className="bg-emerald-500 h-full rounded-full transition-all"
                          style={{
                            width: `${Math.min(
                              (product.stock_quantity / 20) * 100,
                              100
                            )}%`,
                          }}
                        />
                ...
                {/* Description */}
                <p className="mt-5 max-w-full wrap-break-word text-sm text-slate-600 leading-relaxed">
                  {product.description ||
                    'This unique rural product is handcrafted by skilled traditional artisans using eco-friendly materials and heritage techniques passed down through generations.'}
                </p>
              ...
              {/* Quantity + Actions */}
              <div className="min-w-0 space-y-4 pt-4 border-t border-slate-100">
            ...
            "
                ...
                {/* Message */}
                {message && (
                  <div className="w-full min-w-0 p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-semibold flex items-start gap-2">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-3 gap-3 pt-4 border-t border-slate-100 text-center min-w-0">
                ...
                <div className="min-w-0 p-3 rounded-xl bg-sky-50/50 border border-sky-100">
                  <Award className="w-5 h-5 text-sky-600 mx-auto mb-1" />

                  <span className="text-[11px] font-bold text-slate-800 block wrap-break-word">
                    Artisan Certified
                  </span>
                </div>

                <div className="min-w-0 p-3 rounded-xl bg-emerald-50/50 border border-emerald-100">
                ...
                <div className="min-w-0 p-3 rounded-xl bg-indigo-50/50 border border-indigo-100">
                  <Truck className="w-5 h-5 text-indigo-600 mx-auto mb-1" />

                  <span className="text-[11px] font-bold text-slate-800 block break-wordsf">
                    Express Delivery
                  </span>
                </div>
